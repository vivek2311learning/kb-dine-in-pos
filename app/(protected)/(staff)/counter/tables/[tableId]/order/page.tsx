...
export default function OrderPage() {
  ...
  const goToBill = ()=>{
    ...
    const servedItems = orderItems.filter(i=>i.served)
  ...
  const hasServedItems = orderItems.some(i=>i.served)

  const forceCloseTable = async()=>{

    if(!orderId) return

    if(hasServedItems){
      show('error','Items already served. Bill first.')
      return
    }

    const ok = window.confirm('Cancel order and free table?')
    ...
    try{
      ...
      if(!res.ok){
        show('error','Failed to close table')
        return
      }
            ...
            <div className="flex justify-between items-center">
              ...
              {item.kitchenStatus==='draft' ? (
                ...
                <div className="flex gap-2">
                  ...
                  <Button onClick={()=>confirmItem(item._id)}>Confirm</Button>

                  <Button className="bg-red-600" onClick={()=>cancelItem(item._id)}>
                    Cancel
                  </Button>
                ...
                <div className="flex gap-2">
                  ...
                  {!item.served && item.kitchenStatus==='ready' &&
                    <Button className="bg-green-600" onClick={()=>markServed(item._id)}>
                      Served
                    </Button>
                  }
